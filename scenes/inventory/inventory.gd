...
func AdjustBackgroundSize():
	...
	var topMargin = 8
	var minHeight = 0 
	_backgroundWidth = leftMargin + (_columnCount * slotSize) + (_inventoryGrid.get_constant("hseparation") * (_columnCount))
	var calculatedRowCount = ceil(_slotCount / _columnCount) # 15 rounds up to 16
	_backgroundHeight = topMargin + (calculatedRowCount * slotSize) + (_inventoryGrid.get_constant("vseparation") * calculatedRowCount)
	if _backgroundHeight < minHeight:
		_backgroundHeight = minHeight
	_background.rect_size = Vector2(_backgroundWidth, _backgroundHeight)
